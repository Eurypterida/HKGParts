...
function fetchFroza(part) {
  ...
  function parseFrozaJson(jsonStr) {
    ...
    try {
      ...
      for (var b = 0; b < brands.length; b++) {
        var brandName = brands[b];
        var partData = resp.data[brandName];
      ...
      console.log('[Froza] Parse error:', e.message);
    }
    return prices;
  }
  ...
  return httpGet(searchUrl, {
    ...
    var cookies = (pageRes.headers['set-cookie'] || []).map(function(c) { return c.split(';')[0]; }).join('; ');
